...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validasi participant1 (opsional, tapi jika ada harus lengkap)
    if (participant1 && (participant1.name || participant1.phone || participant1.email)) {
      if (!participant1.name || !participant1.phone || !participant1.email) {
        return NextResponse.json({ error: 'Jika mengisi Peserta 1, mohon lengkapi semua data' }, { status: 400 });
      }
    }

    // Validasi participant2 (opsional, tapi jika ada harus lengkap)
    if (participant2 && (participant2.name || participant2.phone || participant2.email)) {
      if (!participant2.name || !participant2.phone || !participant2.email) {
    ...
    const insertData = {
      ...
      attending,
      // Sales Training Participant 1 (opsional, tapi kolom DB adalah NOT NULL dengan default '')
      st_participant_01_name: participant1 && participant1.name ? participant1.name.trim() : '',
      st_participant_01_phone: participant1 && participant1.phone ? participant1.phone.trim() : '',
      st_participant_01_email: participant1 && participant1.email ? participant1.email.trim() : '',
      st_participant_01_food_preference: '', // Disabled for now, set to empty string
      // Sales Training Participant 2 (opsional, kolom DB adalah nullable)
      ...
      st_participant_02_food_preference: null // Disabled for now
    };
